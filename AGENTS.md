# Agent entrypoint

Start with [the latest decision summary](docs/journal/2026-07-29-public-release-r5-summary.md). Open its linked transcript only when exact session context is needed.

Do not commit or push without explicit user authorization. Rebuild `catalog.json` with `node scripts/build-catalog.mjs` in a network-enabled environment before release.
