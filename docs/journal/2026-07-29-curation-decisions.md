...
### 作曲家识别：\b 单词边界在这个场景下反复出错

同一类 bug 栽了三次，每次表现不同，根源都是 `\b` 的定义与真实数据不符：

| 数据形态 | 失败原因 | 修法 |
|---|---|---|
| `johann_sebastian_bach_toccata` | **下划线是单词字符**，`_bach_` 两侧无边界 | 匹配前把 `_` 归一化成空格 |
| `Brahms1`、`Rachmaninoff2` | **字母与数字之间无边界**，结尾的 `\b` 失效 | 结尾改用 `(?![a-z])` |
| `7Handel3` | 同上，**开头**的 `\b` 也失效 | 开头改用 `(?<![a-z])` |

最终规则：`(?<![a-z])名字(?![a-z])` + 先做下划线归一化。
两侧只排除**字母**，所以数字/符号相邻时能命中，而 `Offenbach`、`Bacharach`、
`Brahmsian` 仍不会被误判。`JSBACH` 这种字母缩写粘连的另写别名 `js\s*bach` 覆盖。

**另一条**：识别名单里只能放**作曲家**。Pandora 的路径里有 Goldstein、Hokanson、
Doan、Tipton 等演奏家名字，若把他们加进名单会张冠李戴地署名。

### Pandora Records：接入非 archive.org 源的注意事项

- 目录页是 HTML 3.2 风格，链接标签是**大写** `<A HREF=...>`，正则要 `/i`
- 目录里大量**符号链接**指向 `../` 别处（`piano/Goldstein/` 指向
  `historical_instruments/`），跟随会重复计数 —— 必须跳过 `..` 开头的链接
- **没有元数据接口，时长要自己算**：`HEAD` 取 `content-length` +
  `Range: bytes=0-8191` 解析 MPEG 帧头拿比特率，
  时长 = `content-length × 8 ÷ (比特率 × 1000)`。已用 128/192/256kbps 三档交叉验证。
